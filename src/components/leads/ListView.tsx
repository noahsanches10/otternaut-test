...
export function ListView({
  ...
  const handleBulkArchive = async () => {
    ...
    if (!confirm(`Are you sure you want to archive ${selectedLeads.size} lead${selectedLeads.size > 1 ? 's' : ''}?\n\nThis will move them to the archive.\n\nNote: For any leads that don't archive, check if its "Interested Service Type" needs to be updated.`)) return;
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`${successCount} lead${successCount > 1 ? 's' : ''} archived successfully`);
      }
      if (errorCount > 0) {
        toast.error(`Failed to archive ${errorCount} lead${errorCount > 1 ? 's' : ''}`);
      }
  ...
  const handleBulkRestore = async () => {
    ...
    if (!confirm(`Are you sure you want to restore ${selectedLeads.size} lead${selectedLeads.size > 1 ? 's' : ''}? This action cannot be undone.`)) return;
    ...
    try {
      ...
      await Promise.all(Array.from(selectedLeads).map(id => onRestoreLead(id)));
      setSelectedLeads(new Set());
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`⚠️ WARNING: Are you sure you want to PERMANENTLY DELETE ${selectedLeads.size} lead${selectedLeads.size > 1 ? 's' : ''}?\n\nThis action CANNOT be undone and will permanently delete:\n- All lead data\n- Associated interactions\n- Associated tasks\n\nNote: For any leads that don't delete, check if its "Interested Service Type" needs to be updated.`)) return;
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`${successCount} lead${successCount > 1 ? 's' : ''} deleted successfully`);
      }
      if (errorCount > 0) {
        toast.error(`Failed to delete ${errorCount} lead${errorCount > 1 ? 's' : ''}`);
      }
