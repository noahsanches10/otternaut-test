...
function downloadCustomersAsCsv(customers: Customer[]) {
  ...
  const csvContent = [
    ...
    ...customers.map(customer => [
      ...
      `"${customer.property_state || ''}"`,
      `"${customer.property_zip || ''}"`,
...
export function ListView({
  ...
  const handleBulkArchive = async () => {
    ...
    if (!confirm(`Are you sure you want to archive ${selectedCustomers.size} customer${selectedCustomers.size > 1 ? 's' : ''}?\n\nThis will move them to the archive.\n\nNote: For any customers that don't archive, check if its "Service Type" needs to be updated.`)) return;
    
    try {
      let successCount = 0;
      let errorCount = 0;

      // Process each customer individually to handle errors gracefully
      ...
      if (successCount > 0) {
        toast.success(`${successCount} customer${successCount > 1 ? 's' : ''} archived successfully`);
      }
      if (errorCount > 0) {
        toast.error(`Failed to archive ${errorCount} customer${errorCount > 1 ? 's' : ''}`);
      }
  ...
  const handleBulkRestore = async () => {
    ...
    if (!confirm(`Are you sure you want to restore ${selectedCustomers.size} customer${selectedCustomers.size > 1 ? 's' : ''}? This action cannot be undone.`)) return;
    ...
    try {
      ...
      // Then update UI and trigger refresh
      await Promise.all(Array.from(selectedCustomers).map(id => onRestoreCustomer(id)));
      setSelectedCustomers(new Set());
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`⚠️ WARNING: Are you sure you want to PERMANENTLY DELETE ${selectedCustomers.size} customer${selectedCustomers.size > 1 ? 's' : ''}?\n\nThis action CANNOT be undone and will permanently delete:\n- All customer data\n- Associated interactions\n- Associated tasks\n\nNote: For any customers that don't delete, check if its "Service Type" needs to be updated.`)) return;
