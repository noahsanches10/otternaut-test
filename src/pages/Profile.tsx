...
import { Plus, X, Save, RotateCcw, Building2, Settings2, Calculator, UserCog } from 'lucide-react';
...
export function Profile() {
  ...
  const [activeTab, setActiveTab] = useState(() => 
    searchParams.get('reset') === 'true' ? 'account' : 'business'
  );
  ...
  useEffect(() => {
    if (profile) {
      ...
      setBusinessInfo({
        ...
        personal_linkedin: profile.personal_linkedin || '',
  ...
  async function handleSaveBusinessInfo() {
    ...
    const hasRequiredFields = personalInfo.firstName && personalInfo.lastName && personalInfo.companyName;
  ...
  useEffect(() => {
    if (profile) {
      ...
      setEditedSources(profile.lead_sources || [...DEFAULT_LEAD_SOURCES]);
      setEditedStages(profile.lead_stages || [...DEFAULT_LEAD_STATUSES]);
      setEditedServiceTypes(profile.service_types || [...DEFAULT_SERVICE_TYPES]);
      setEditedFrequencies(profile.service_frequencies || DEFAULT_FREQUENCIES.map(f => f.name));
    }
  }, [profile]);
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sign out to require new login with new password
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-bold text-foreground -mt-2">Profile Settings</h1>
        ...
        <TabsContent value="business" className="mt-6">
          <div className="bg-card p-6 rounded-lg border border-border shadow-sm space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <div className="col-span-2 grid grid-cols-3 gap-6 pb-6 border-b border-border">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="John"
                    className={cn(
                      profile?.first_name && "bg-muted text-muted-foreground cursor-not-allowed"
                    )}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Doe"
                    className={cn(
                      profile?.last_name && "bg-muted text-muted-foreground cursor-not-allowed"
                    )}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={cn(
                      profile?.company_name && "bg-muted text-muted-foreground cursor-not-allowed"
                    )}
